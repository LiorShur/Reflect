...
export function useAppreciationFeed(uid: string | null): AppreciationFeedView {
  ...
  useEffect(() => {
    ...
    // RTDB lacks a "since X" filter without an additional condition,
    // so we use orderByChild + endAt(now). The 90-day window is then
    // applied client-side; small data volume per user makes this
    // acceptable and avoids a second indexed query.
    const r = query(
